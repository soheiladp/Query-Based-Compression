...
algorithms = {
                ...
                'my': (None, None, 'my', [], None),#'mydawa': ('l1approx', [], 'greedyH', [], ratio),
			 }


def AlgorithmBuilder(alg, p_name, p_argv, e_name, e_argv, ratio, weights, seed, stdb, split_flag):
    ...
    if p_name is None:
        ...
        p_engine = partition_engine.registry[p_name]
        return e_engine.engine(alg, p_engine(*p_argv), e_engine.estimate(*e_argv), ratio, None, weights,
                               seed, stdb, split_flag)


#def Algorithm(name):
def Algorithm(alg, p_name, p_argv, e_name, e_argv, ratio, weights = [], seed=500, stdb = 0.5, split_flag = True):
    """Build an algorithm with predefined configuations in dict `algorithm'."""
    #return AlgorithmBuilder(*algorithms[name.lower()])
    return AlgorithmBuilder(alg, p_name, p_argv, e_name, e_argv, ratio, weights, seed, stdb, split_flag)
