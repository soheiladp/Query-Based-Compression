...
// n size of x
void L1partition_approx(int *hist, int n1, int* x, int n, double epsilon, int* weights, int w, double ratio, long seed)    {
    ...
    while ( len <= n )   {
        ...
        node* root = new node(x[0]/weights[0]);
        long sum = x[0]/weights[0];
        double invlen = 1.0/len;
        for (int i=1; i<len; i++) {
            insert(root, x[i]/weights[i]);
            sum += x[i]/weights[i];
        }
        for (int i=len-1; i<n; i++) {
            ...
            delete loc;
            if ( i < n-1 )  {
                sum += x[i+1]/weights[i+1] - x[i-len+1]/weights[i-len+1];
                insert(root, x[i+1]/weights[i+1]);
                remove(root, x[i-len+1]/weights[i-len+1]);
            }
        }
        len <<= 1;
        off ++;
...
double* cumabserr(int *x, int n, int* weights)   {
...
void L1partition(int *hist, int n1, int* x, int n, double epsilon, int* weights, int w, double ratio, long seed)    {
    ...
    rnum.seed(seed);
    for (int i=0; i<n; i++) {
        score[i] = cumabserr(x+i, n-i, weights+i);
        for (int j=0; j<n-i; j++)   {
            double r = 0.5 - rnum();
            double lap =  0;
            if ( j > 0 )
