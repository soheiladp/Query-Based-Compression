...
vector<int> DP::adjustBoundery(vector<double>& hist, double epsilon1, double maxF){
...
void DP::outputNoisyCounts_StructureFirst(vector<double>& counts, vector<double>& cutCounts, 
								  double epsilon1, double epsilon2, double maxF, vector<double>& ovec){
    int ct = 0;
	run(cutCounts);
	vector<int> boundery = adjustBoundery(cutCounts, epsilon1, maxF);///use \epsilon_1 budget
	if((boundery.size() == counts.size() + 1) || (boundery.size() == 2))
		epsilon2 = epsilon1 + epsilon2;
	for(vector<int>::size_type i = 0; i < boundery.size() - 1; i ++){
		const vector<double>& leaves = boost(counts, boundery[i], boundery[i+1], epsilon2);
...
vector<double> DP::boost(vector<double>& ori_hist, int begin, int end, double epsilon2){
	int fan_out = 2;
	double hay_epsilon = epsilon2;
	if(end == begin + 1){
		double value = ori_hist[begin] + Utilities::getLapNoise((1.0)/epsilon2);
		vector<double> a;
		a.push_back(value);
		return a;
	}

	HTree htree(fan_out, ori_hist, begin, end, hay_epsilon);
	htree.inference();
