...
void M_DP::outputNoisyCounts_M_StructureFirst(vector<double>& counts, 
								  double epsilon1, double epsilon2, ofstream& o_file){
	run(counts);
	vector<int> boundary;
	boundary = adjustBoundary(counts, epsilon1);
	if((boundary.size() == counts.size() + 1) || (boundary.size() == 2))
		epsilon2 = epsilon1 + epsilon2;
	for(vector<int>::size_type i = 0; i < boundary.size() - 1; i ++){
		const vector<double>& leaves = boost(counts, boundary[i], boundary[i+1], epsilon2);
...
vector<int> M_DP::adjustBoundary(vector<double>& hist, double epsilon1){
...
vector<double> M_DP::boost(vector<double>& ori_hist, int begin, int end, double epsilon2){
	int fan_out = 2;
